              ...
              <div class="space-y-6">
                <div class="flex items-center justify-between gap-4">
                  ...
                  <span
                    ...
                    class="size-5 animate-spin rounded-full border-2 border-slate-200 border-t-primary dark:border-slate-700 dark:border-t-primary"
                  />
                </div>

                <div v-if="externalBindings.length" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div
                    ...
                    <div class="flex items-start justify-between gap-3">
                      <div class="min-w-0">
                        ...
                        <p class="external-binding-item__meta">
                          {{ binding.bound ? (binding.email || binding.displayName || '已绑定') : '未绑定' }}
                        </p>
                      </div>
                      <span
                ...
                <div
                  v-else
                  class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-xs font-medium text-slate-500 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-400"
                >
                  暂无可绑定的第三方登录方式
                </div>
              </div>
...
:global(html.dark .external-binding-item) {
  border-color: var(--wk-border-subtle);
  background: var(--wk-bg-surface-subtle);
}

:global(html.dark .external-binding-item__name) {
  color: var(--wk-text-primary);
}

:global(html.dark .external-binding-item__meta) {
  color: var(--wk-text-muted);
}

:global(html.dark .external-binding-item__status--off) {
  background: var(--wk-border-strong);
}

:global(html.dark .external-binding-item__action) {
  background: rgb(var(--wk-primary-rgb) / 0.14);
  color: var(--wk-primary);
}

:global(html.dark .external-binding-item__action:hover:not(:disabled)) {
  background: rgb(var(--wk-primary-rgb) / 0.2);
}

:global(html.dark .external-binding-item__action--danger) {
  background: rgb(244 63 94 / 0.14);
  color: #fb7185;
}

:global(html.dark .external-binding-item__action--danger:hover:not(:disabled)) {
