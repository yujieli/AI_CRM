  ...
  <div class="bg-slate-50/50 -m-4 md:-m-6 p-4 md:p-8">
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden">
      <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-white">
        ...
        <button
          class="bg-primary text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-primary/90 transition-all shadow-sm shadow-primary/20 whitespace-nowrap"
          @click="$emit('add-role')"
        >
        ...
        <template v-else>
          ...
          <div class="md:hidden space-y-3">
            <div
              ...
              :key="role.roleId"
              class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm"
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <div class="flex items-center gap-2 min-w-0">
                    <p class="text-sm font-bold text-slate-900 truncate">{{ role.roleName }}</p>
                    <span
                      v-if="role.realm === 'super_admin'"
                      class="shrink-0 px-2 py-0.5 bg-slate-100 text-slate-500 text-xs font-bold rounded-md uppercase tracking-wider"
                    >系统预设</span>
                  </div>
                ...
                <span class="shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-100 text-slate-600 text-xs font-medium whitespace-nowrap">
                  <span class="material-symbols-outlined text-[14px]">group</span>
                  {{ role.userCount }} 人
                </span>
              </div>
              ...
              <div class="mt-3 flex flex-wrap items-center justify-end gap-2">
                <button
                  type="button"
                  class="h-8 px-3 rounded-xl border border-primary/20 bg-primary/5 text-primary text-xs font-bold whitespace-nowrap"
                  @click="$emit('open-drawer', role, 'members')"
                >
                  管理用户
                </button>
                ...
                <button
                  type="button"
                  class="h-8 px-3 rounded-xl border border-slate-200 text-slate-600 text-xs font-bold whitespace-nowrap"
                  @click="$emit('edit-role', role)"
                >
                  编辑
                </button>
                <button
                  v-if="role.realm !== 'super_admin'"
                  type="button"
                  class="h-8 px-3 rounded-xl border border-slate-200 text-slate-600 text-xs font-bold whitespace-nowrap hover:text-red-600"
                  @click="$emit('delete-role', role)"
                >
                  删除
                </button>
