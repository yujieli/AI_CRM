<template>
  <Teleport to="body">
    <Transition name="project-attachment-preview-fade">
      <div
        v-if="modelValue"
    ...
    <Transition name="project-attachment-preview-scale">
      <div
          ...
          <div class="flex min-w-0 flex-1 flex-col">
            <header class="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 px-4 py-3 sm:px-5">
              <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                  <h3 class="truncate text-sm font-bold text-slate-900">{{ attachment.name || 'Attachment preview' }}</h3>
            ...
            <main class="relative min-h-0 flex-1 overflow-hidden bg-slate-50">
              ...
              <div v-else class="flex h-full items-center justify-center p-6">
                <div class="max-w-sm text-center text-slate-500">
                  ...
                  <p class="text-sm font-medium">This attachment type cannot be previewed in the browser.</p>
                  <button
                    type="button"
                    class="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-primary/90"
                    @click="handleDownload"
...
import { downloadProjectTaskAttachment, getProjectTaskAttachmentBlob } from '@/api/project'
...
async function handleDownload() {
  if (!props.attachment || !props.projectId || !props.taskId) return
  try {
    await downloadProjectTaskAttachment(
      props.projectId,
      props.taskId,
      props.attachment.attachmentId,
      props.attachment.name || 'attachment'
    )
  } catch (error) {
    console.error('Download project task attachment failed:', error)
    ElMessage.error('Attachment download failed')
  }
}
...
async function loadPreview(attachment: ProjectTaskAttachment) {
  ...
  if (!props.projectId || !props.taskId || !attachment.attachmentId) {
    errorMessage.value = 'Attachment information is incomplete.'
    return
  }
  if (previewKind.value === 'unsupported') {
    errorMessage.value = 'This attachment type cannot be previewed in the browser. Please download it instead.'
    return
  }

  loading.value = true
  try {
    const blob = await getProjectTaskAttachmentBlob(props.projectId, props.taskId, attachment.attachmentId)
    if (previewKind.value === 'text') {
      if (blob.size > TEXT_PREVIEW_MAX_BYTES) {
        errorMessage.value = 'This text attachment is too large to preview. Please download it instead.'
        return
      }
    ...
    console.error('Preview project task attachment failed:', error)
    errorMessage.value = 'Attachment preview failed. Please download it instead.'
  } finally {
    loading.value = false
  }
}
...
function resolvePreviewKind(attachment: ProjectTaskAttachment): PreviewKind {
...
<style scoped>
.project-attachment-preview-fade-enter-active,
.project-attachment-preview-fade-leave-active,
.project-attachment-preview-scale-enter-active,
.project-attachment-preview-scale-leave-active {
  transition: opacity 0.16s ease, transform 0.16s ease;
}

.project-attachment-preview-fade-enter-from,
.project-attachment-preview-fade-leave-to {
  opacity: 0;
}

.project-attachment-preview-scale-enter-from,
.project-attachment-preview-scale-leave-to {
  opacity: 0;
