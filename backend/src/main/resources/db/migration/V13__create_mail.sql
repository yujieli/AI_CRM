...
CREATE UNIQUE INDEX IF NOT EXISTS uk_mail_account_user_email
    ON crm_mail_account (user_id, lower(email_address));
CREATE INDEX IF NOT EXISTS idx_mail_account_user ON crm_mail_account (user_id);
CREATE INDEX IF NOT EXISTS idx_mail_account_sync ON crm_mail_account (enabled, last_sync_time);

DROP TRIGGER IF EXISTS trg_mail_account_update_time ON crm_mail_account;
...
CREATE INDEX IF NOT EXISTS idx_mail_message_customer ON crm_mail_message (customer_id, received_time DESC);
CREATE INDEX IF NOT EXISTS idx_mail_message_user_time ON crm_mail_message (user_id, received_time DESC);
CREATE INDEX IF NOT EXISTS idx_mail_message_account_dir ON crm_mail_message (account_id, direction, received_time DESC);
CREATE INDEX IF NOT EXISTS idx_mail_message_subject ON crm_mail_message (subject);
CREATE INDEX IF NOT EXISTS idx_mail_message_from ON crm_mail_message (lower(from_address));
CREATE INDEX IF NOT EXISTS idx_mail_message_thread ON crm_mail_message (account_id, thread_id);

DROP TRIGGER IF EXISTS trg_mail_message_update_time ON crm_mail_message;
...
DROP TRIGGER IF EXISTS trg_mail_attachment_update_time ON crm_mail_attachment;
...
CREATE INDEX IF NOT EXISTS idx_mail_sync_log_account_time
    ON crm_mail_sync_log (account_id, started_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_mail_draft_customer ON crm_mail_draft (customer_id, create_time DESC);
CREATE INDEX IF NOT EXISTS idx_mail_draft_user ON crm_mail_draft (user_id, create_time DESC);

DROP TRIGGER IF EXISTS trg_mail_draft_update_time ON crm_mail_draft;
...
CREATE INDEX IF NOT EXISTS idx_mail_template_user ON crm_mail_template (user_id, update_time DESC);
CREATE INDEX IF NOT EXISTS idx_mail_template_category ON crm_mail_template (user_id, category);

DROP TRIGGER IF EXISTS trg_mail_template_update_time ON crm_mail_template;
...
DROP TRIGGER IF EXISTS trg_mail_sync_cursor_update_time ON crm_mail_sync_cursor;
...
SELECT 2300000000000 + m.menu_id,
       r.role_id,
       m.menu_id,
       5,
       CURRENT_TIMESTAMP
FROM manager_role r
JOIN manager_menu m ON m.menu_id IN (2300, 2301, 2302, 2303, 2304)
