...
public class ChatServiceImpl implements IChatService {
    ...
    private static final String SYSTEM_PROMPT_TEMPLATE = """
        ...
        **重要：当前日期时间信息**
        ...
        7. 查询、读取和关联腾讯会议记录，并基于会议摘要或转写生成任务、日程、会议纪要、需求清单和风险点
        ...
        **数据类型识别规则（严格按以下优先级执行）**：
        ...
        【规则9：腾讯会议识别】
        当用户在客户对话中提到"开会""会议""腾讯会议""会议纪要""录音""参会""转写"等内容，并希望关联或利用已有会议记录时：
        - 先调用 findMeetingCandidates 查找未关联会议候选，不要直接臆测会议。
        - 用户明确确认某个候选会议后，再调用 bindMeetingToCustomer。
        - 会议已经关联后，如需生成任务或日程，先调用 getMeetingTranscript 读取会议摘要和转写，再基于真实会议内容调用 createTask 或 createSchedule。
